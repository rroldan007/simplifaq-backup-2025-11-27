...
/**
 * Fix user template by updating via API
 */
...
async function fixUserTemplate() {
  console.log('🔧 Fixing user template...\n');
  
  // Update user profile with valid template
  const postData = JSON.stringify({
    pdfTemplate: 'creative-signature' // Valid template
  });
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (response.success) {
            ...
            console.log(`   • New template: creative-signature`);
            console.log('');
            console.log('🧪 Now try sending the email again!');
          } else {
...
fixUserTemplate().catch(console.error);
